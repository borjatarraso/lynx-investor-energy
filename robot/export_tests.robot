...
Export TXT With Energy Quality Field
    [Documentation]    GIVEN a report with energy_quality WHEN I export as TXT THEN quality data is included
    When I Run Python Code "from lynx_energy.models import AnalysisReport, CompanyProfile, CompanyTier, CompanyStage, EnergyQualityIndicators; from lynx_energy.export import export_report, ExportFormat; from pathlib import Path; import tempfile; p = CompanyProfile(ticker='EQ', name='EQ Energy Corp', sector='Energy', industry='Oil & Gas E&P', country='Canada', market_cap=100000000); p.tier = CompanyTier.MICRO; p.stage = CompanyStage.PRODUCER; eq = EnergyQualityIndicators(quality_score=82.0, reserve_quality='High', reserve_life_assessment='15+ years', production_scale_assessment='Mid-scale producer'); r = AnalysisReport(profile=p, energy_quality=eq); f = tempfile.NamedTemporaryFile(suffix='.txt', delete=False); path = export_report(r, ExportFormat.TXT, Path(f.name)); content = path.read_text(); assert 'EQ Energy Corp' in content; assert 'Quality' in content or 'quality' in content; print('OK')"
    Then The Exit Code Should Be 0
    Then The Output Should Contain "OK"
